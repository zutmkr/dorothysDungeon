...
def generator():
    ...
    f = LabelFrame(okno, text='GENERATED MAP', labelanchor='n')
    f.grid(columnspan=3, row=0)

    f2 = LabelFrame(okno, text='DUNGEON OPTIONS', labelanchor='n')
    ...
    wcisniety = IntVar()
    wcisniety.set(0)
    c = Checkbutton(okno, text='Randomize all parameters', variable=wcisniety, command=losuj)
    ...
    b1 = Button(okno, text='GENERATE', bd=4, width=15, height=2, command=generuj_mape)
    b1.grid(column=5, row=2)

    b2 = Button(okno, text='SAVE MAP', bd=4, width=15, height=2, command=lambda: zapisz_mape(mapa_odkryta))
    b2.grid(column=5, row=3)

    b3 = Button(okno, text='PLAY', bd=4, width=15, height=2, bg="light blue", command=lambda: zagraj(poziom_pgen, wielkosc_mapygen, punkty_zycia, sila, okno))
    b3.grid(column=6, row=2)

    b4 = Button(okno, text='EXIT', bd=8, width=20, height=5, command=lambda: wyjdz(okno))
    b4.grid(column=7, row=7)

    b5 = Button(okno, text='LOAD MAP', bd=5, width=20, height=2, command=lambda: wczytaj_mape(mapa_odkryta))
